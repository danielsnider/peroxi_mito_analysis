function [raw_differences, normalized_differences, composite_differences] = DifferentialMeasurements(SubsetTable)
  ...
  log_msg(sprintf('[%s]: %s', mfilename(), 'Measuring differences...'));
  ...
  % For Peroxisome tracking
  weights = {};
  weights.PeroCentroid = 1;
  % weights.PeroArea = 1;
  ...
  for t=min(SubsetTable.Timepoint):max(SubsetTable.Timepoint)-1
    % fprintf('Calculating differences between frames %d and %d...\n', t, t+1)

    % Get only cells (ie. table rows) at T and T+1
    T1 = SubsetTable(SubsetTable.Timepoint==t,:);
    T2 = SubsetTable(SubsetTable.Timepoint==t+1,:);
