...
for typ=fields(s)'
  ...
  for sid=1:length(s.(typ))
    ...
    for tid=1:timepoints
      ...
      if SAVE_TO_DISK
        ...
        if strcmp(tracked,'tracked')
          fig_name = sprintf('/3D distance_visualization type_%s cell_%03d timepoint_%03d %s',typ, stack_id, tid, tracked);
          ...
          % Top Down View
          fig_name = sprintf('/3D distance_visualization type_%s cell_%03d  top_down_view timepoint_%03d %s',typ, stack_id, tid, tracked);
          view(2)
          set(gca,'Color',[0 0 0 ]);
          set(gcf,'Color',[0 0 0 ]);
          axis off
          ...
          if ~exist('timelapse_gif_top_down')
            timelapse_gif_top_down=uint8(zeros(size(imageData,1),size(imageData,2),3,timepoints));
          end

          % Resize images to be the minimum common size in the X and Y
          size_imageData = size(imageData) % debug only
          min_x_res = min(size(timelapse_gif_top_down,1), size(imageData,1));
          min_y_res = min(size(timelapse_gif_top_down,2), size(imageData,2));
          timelapse_gif_top_down = timelapse_gif_top_down(1:min_x_res, 1:min_y_res, :,:); % minimum common size
          imageData = imageData(1:min_x_res,1:min_y_res,:); % minimum common size


          timelapse_gif_top_down(:,:,:,tid) = imageData;
    ...
    if SAVE_TO_DISK
      if strcmp(tracked,'tracked')
        fig_name = sprintf('/0_3D_gif_distance_visualization type_%s cell_%03d %s.gif', typ, stack_id, tracked);
        save_path = [fig_save_path fig_name];
        colour_imgs_to_gif(timelapse_gif,save_path, 1/2);
        fig_name = sprintf('/0_3D_gif_distance_visualization type_%s cell_%03d top_down_view %s.gif', typ, stack_id, tracked);
        save_path = [fig_save_path fig_name];
        colour_imgs_to_gif(timelapse_gif_top_down,save_path, 1/2);
      end
    end
